...
TestForge AI is a **TypeScript monorepo** (~75 source files) organized as a **Turborepo + Bun workspace** with **8 packages** and **1 Fastify API application**. It delivers AI-powered test automation across **web**, **mobile**, and **API** surfaces — with built-in intelligence for test generation, self-healing locators, failure analysis, visual regression, accessibility auditing, and regulatory compliance.

**Build status:** All 9 packages compile cleanly under TypeScript strict mode (`bun run build` → 9/9 successful).
...
├── packages/
│   ├── core/               # Zod schemas, typed EventBus, LicenseManager
│   ├── ai-engine/          # 12 AI modules: providers, generators, analyzers, agents
│   ├── cli/                # Commander CLI with 10 commands
│   ├── web-runner/         # Playwright-based web test runner
│   ├── mobile-runner/      # Appium / WebdriverIO mobile test runner
│   ├── api-runner/         # REST, GraphQL, gRPC, WebSocket API test runner
...
```
  ...
  apps/api ──► @testforge/core (schemas, events, license)
```
...
### `@testforge/ai-engine` — 24 files (12 modules, 7 test suites)
...
These are gaps identified from the codebase — no implementation exists yet.
...
### Frontend / Dashboard
- **Web UI** — no React/Next.js frontend exists; API-only interaction
- **WebSocket client** — server-side WebSocket upgrade exists, no client-side consumer
...
cd testforge-ai
bun install
bun run build          # 9/9 packages successful
```
...
| Command | Description |
|---|---|
| `bun run build` | Build all 9 packages (Turborepo) |
| `bun run dev` | Watch mode for all packages |
| `bun run test` | Run Vitest unit tests |
| `bun run lint` | ESLint (flat config, typescript-eslint strict) |
| `bun run typecheck` | TypeScript type checking |
...
### Collaboration & UX
- Web dashboard (React/Next.js) for test management
- Browser extension for test recording
- Shared step/fixture library across teams
- Multi-tenant organization isolation
...
*Built with [Turborepo](https://turbo.build/) + [Bun](https://bun.sh/) + [Fastify](https://fastify.dev/) + [Playwright](https://playwright.dev/) + [Anthropic Claude](https://www.anthropic.com/)*
