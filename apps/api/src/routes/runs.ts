...
export async function registerRunRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get("/api/runs/:id/stream", {
    ...
    handler: async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const raw = reply.raw;
      raw.write(`data: ${JSON.stringify({ event: "connected", runId: params.id })}\n\n`);

      // In production, subscribe to BullMQ queue events or WebSocket broadcasts
      // and stream them as SSE events here
    },
  });
  ...
  app.get("/api/runs", {
    ...
    handler: async () => {
      return Array.from(runs.values()).slice(0, 20);
    },
  });
}
