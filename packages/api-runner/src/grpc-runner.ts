...
/**
 * GrpcRunner executes gRPC service methods
 *
 * Features:
 * - Loads .proto files dynamically
 * - Executes unary and streaming calls
 * - Validates response structure
 *
 * @example
 * ```ts
 * const runner = new GrpcRunner(eventBus, { address: "localhost:50051" });
...
export class GrpcRunner {
  ...
  private async _executeStep(step: TestStep): Promise<StepResult> {
    ...
    try {
      // In production: use @grpc/grpc-js to make the actual call
      // const client = new grpcClient(data.address, protoPackage);
      // const response = await client[data.method](data.request);

      log("gRPC call: %s at %s", data.method, data.address ?? this._config.address);

      // Simulate call for structure
   ...
   */
  async loadProto(protoPath: string): Promise<Record<string, unknown>> {
    // In production: use @grpc/proto-loader to load
    log("Loading proto file: %s", protoPath);
